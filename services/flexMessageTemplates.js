...
class FlexMessageTemplates {
  ...
  static createTransactionList(data) {
    ...
    const transactionItems = transactions.slice(0, 10).map(item => ({
      ...
      contents: [
        ...
        {
          type: "text",
          text: `${item.ประเภท === 'รายรับ' ? '+' : '-'}${item.จำนวน.toLocaleString()}`,
          weight: "bold",
  ...
  static createMonthlySummary(data) {
    const { month, year, income, expense, balance, topCategories, totalTransactions } = data;
  ...
  static createBalanceReport(data) {
    const { currentBalance, monthlyAverage, savingsRate, topExpenseCategory } = data;
          ...
          contents: [
            // ยอดคงเหลือปัจจุบัน
            {
              ...
              contents: [
                ...
                {
                  type: "text",
                  text: `${currentBalance >= 0 ? '+' : ''}${currentBalance.toLocaleString()} บาท`,
                  weight: "bold",
                  size: "xl",
